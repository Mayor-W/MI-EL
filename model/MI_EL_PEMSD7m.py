...
class TemporalEmbedding(nn.Module):
    def __init__(self, TimeInfo, num_embeddings, timeembeddings_k, d_model, max_len, num_timesteps_output):
        ...
        self.linear_1 = nn.Linear((len(num_embeddings) + 1) * timeembeddings_k, 18 * d_model)
        self.linear_2 = nn.Linear(18 * d_model, num_timesteps_output * d_model)
        self.act = nn.ReLU()
    ...
    def forward(self, indices):
        ...
        self.time_attitude = torch.from_numpy(self.TimeInfo.iloc[indices].values[:, 1:].astype(np.float64)).type(
            torch.long).to(device='cuda')  # [month,week,day,hour..] batchsize*num_embeddings

        # temporal position embedding
        sincos_pe = self.pe[indices.numpy(), :]  # batchsize*d_model

        # Concatenate the embeddings
        # print(time_attitude.device, sincos_pe.device)
        inputs_embedding = torch.cat([self.embedding_modules[i](self.time_attitude[:, i])
...
class SpatialEmbedding(nn.Module):
    def __init__(self, eigenmaps_k, d_model):
        ...
        self.linear_1 = nn.Linear(eigenmaps_k, 8 * d_model)
        self.linear_2 = nn.Linear(8 * d_model, d_model)
        self.act = nn.ReLU()
        self.drop = nn.Dropout(0.00)
