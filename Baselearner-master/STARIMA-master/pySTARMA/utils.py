...
def prediction(ts_matrix, wa_matrices, phi, theta, prediction_lag=1):
    ...
    predictions = ts_matrix.copy() # 12:36
    residuals = residuals_estimation(predictions, wa_matrices, phi, theta) # 12:36

    # 对每一个未来的时间步
    for h in range(prediction_lag):
        # 定义未来第h步的各个空间节点的预测值
        predict = np.zeros([1, len(phi[0])]) # 1*36

        # 遍历每个空间节点
        for slag in range(len(phi[0])):
            # 遍历每个历史相关时间步
            for tlag in range(len(phi)):
                predict[:,slag] += (predictions[[- tlag - 1]] * phi[tlag, slag]).dot(wa_matrices[slag].T)

            # 遍历每个平均步
            for tlag in range(len(theta)):
                predict[:,slag] -= (residuals[[- tlag - 1]] * theta[tlag, slag]).dot(wa_matrices[slag].T)
    ...
    #         # 遍历每个空间节点
    #         for slag in range(len(phi[0])):
    #             # print(wa_matrices[slag].shape)
    #             # print((predictions[[- tlag - 1]] * phi[tlag, slag]).dot(wa_matrices[slag].T).shape)
    #
    ...
    return predictions[-prediction_lag:, :]
...
def set_stationary(ts_matrix, lags):
    ...
    return time_series_matrix.values
